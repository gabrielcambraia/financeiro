...
import { buscarFluxoDeCaixa } from '../api/fluxoDeCaixa'
...
export default function FluxoDeCaixa() {
  ...
  const { data: fluxo, isLoading } = useQuery({
    queryKey: ['fluxo-de-caixa', dias, contaId, simulando ? simValor : null, simulando ? simData : null],
    queryFn: () => buscarFluxoDeCaixa({
      dias,
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
        <h1 className="text-2xl font-bold text-conteudo">Fluxo de caixa</h1>
      ...
      {fluxo && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="card">
            ...
            <p className="text-2xl font-bold text-conteudo">{fmt(fluxo.saldoAtual)}</p>
          </div>
