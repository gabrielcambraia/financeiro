...
import {
  listarFiliais, criarFilial, atualizarFilial, excluirFilial, buscarAssinatura, type DadosFilial
} from '../api/filiais'
...
export default function Filiais() {
  ...
  const { data: filiais = [], isLoading } = useQuery({ queryKey: ['filiais'], queryFn: listarFiliais })
  ...
  const [form, setForm] = useState<DadosFilial>(dadosVazios())
  const [erro, setErro] = useState('')
  const [confirmarExcluir, setConfirmarExcluir] = useState<Filial | null>(null)
  ...
  const criar = useMutation({
    ...
    onSuccess: () => { invalidar(); toast.success('Filial salva'); fecharModal() },
    onError: (e: unknown) => {
  ...
  const atualizar = useMutation({
    mutationFn: ({ id, dados }: { id: number; dados: DadosFilial }) => atualizarFilial(id, dados),
    onSuccess: () => { invalidar(); toast.success('Filial salva'); fecharModal() },
    onError: (e: unknown) => {
      const err = e as { response?: { data?: { mensagem?: string } } }
      setErro(err?.response?.data?.mensagem ?? 'Erro ao salvar')
  ...
  const excluir = useMutation({
    mutationFn: excluirFilial,
    onSuccess: () => { invalidar(); setConfirmarExcluir(null); toast.success('Filial excluída') },
  })
  ...
  const abrirEditar = (fil: Filial) => {
    setEditando(fil)
    setForm({
      tipoPessoa: fil.tipoPessoa,
      nome: fil.nome,
      nomeFantasia: fil.nomeFantasia ?? '',
      documento: fil.documento,
      inscricaoEstadual: fil.inscricaoEstadual ?? '',
      dataNascimento: fil.dataNascimento ?? '',
      email: fil.email ?? '',
      telefone: fil.telefone ? fil.telefone.replace(/\D/g, '') : '',
      cep: fil.cep ? fil.cep.replace(/\D/g, '') : '',
      logradouro: fil.logradouro ?? '',
      numero: fil.numero ?? '',
      complemento: fil.complemento ?? '',
      bairro: fil.bairro ?? '',
      cidade: fil.cidade ?? '',
      uf: fil.uf ?? '',
    })
  ...
  const setF = (k: keyof DadosFilial, v: string) => setForm(f => ({ ...f, [k]: v }))
  ...
  const atingiuLimite = assinatura ? assinatura.filiaisUsadas >= assinatura.limiteFiliais : false
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-conteudo">Filiais</h1>
          {assinatura && (
            <p className="text-sm text-conteudo-suave mt-1">
              {assinatura.filiaisUsadas}/{assinatura.limiteFiliais} filiais • Plano {assinatura.nomePlano}
            </p>
          )}
        </div>
        <button
          ...
          title={atingiuLimite ? `Limite do plano ${assinatura?.nomePlano} atingido` : 'Nova filial'}
      ...
      {isLoading ? (
        ...
        <p className="text-conteudo-suave text-center py-12">Nenhuma filial cadastrada.</p>
      ) : (
        <div className="space-y-3">
          {filiais.map(fil => (
            <div key={fil.id} className="card flex items-center justify-between gap-4">
              <div className="min-w-0">
                <p className="font-medium text-conteudo truncate">{fil.nome}</p>
                <p className="text-sm text-conteudo-suave">
                  {rotuloTipoPessoa(fil.tipoPessoa)} · {formatarDocumento(fil.documento, fil.tipoPessoa)}
                  {fil.cidade && ` · ${fil.cidade}${fil.uf ? `/${fil.uf}` : ''}`}
                </p>
              </div>
              <div className="flex gap-2 shrink-0">
                <button onClick={() => abrirEditar(fil)}
                  className="text-sm text-acento hover:underline">Editar</button>
                <button onClick={() => setConfirmarExcluir(fil)}
      ...
      >
        {`Deseja excluir a filial "${confirmarExcluir?.nome}"? Esta ação não pode ser desfeita.`}
      </ModalConfirmacao>
