...
public class FluxoDeCaixaService {
    ...
    public FluxoDeCaixaDTO projetar(int dias, Long contaId, BigDecimal simulacaoValor, LocalDate simulacaoData) {
        ...
        var fonte = filialId != null
                ? transacaoRepository.findByEspacoIdAndDataVencimentoBetweenFiltradoPorFilial(espacoId, hoje, fim, filialId)
        ...
        List<FluxoDeCaixaDTO.Ponto> pontos = new ArrayList<>();
        ...
        for (int i = 0; i <= dias; i++) {
            ...
            pontos.add(FluxoDeCaixaDTO.Ponto.builder()
                    .data(dia)
                    .saldo(acumulado)
        ...
        return FluxoDeCaixaDTO.builder().saldoAtual(saldoAtual).pontos(pontos).build();
    }
